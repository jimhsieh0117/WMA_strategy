...
class LongTrendStrategy(BaseTrendStrategy):
    ...
    def detect_entry(
        ...
        max_attempts = self.params.entry_retry.long_max_attempts

        # 1. 看 bar_index 是不是新金叉根 → 重設 pending
        if self._detect_cross(df, bar_index):
            self._pending_cross_bar = bar_index
            self._pending_attempts_used = 0

        # 2. 無 pending → 直接 return
        if self._pending_cross_bar is None:
            return None

        # 3. attempts 用完 → 丟棄 pending
    ...
    # --------------------------------------------------------------------- #
    # 拆分後的兩階段：(a) 偵測 WMA 交叉、(b) 檢查其他全部進場條件
    # --------------------------------------------------------------------- #

    def _detect_cross(self, df: pd.DataFrame, bar_index: int) -> bool:
        """是否在 bar_index 形成金叉（WMA_fast 從下穿越 WMA_slow）。"""
        if bar_index < 1:
            return False
    ...
    def _try_entry(
        ...
        # chop_filter（BBW_rank / ATR_rank / ADX）
        if not passes_chop_filter(df, bar_index, self.params):
            return None

        # structure_filter（market structure 順勢）
        if not passes_structure_filter(
            df, bar_index, Direction.LONG, self.params,
        ):
            return None
        ...
        return EntrySignal(
            ...
            reason=(
                f"long entry (attempt {self._pending_attempts_used}, "
