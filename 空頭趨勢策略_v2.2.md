...
> **方向**：做空（Short）
> **最後更新**：2026-05-13（v2.3 — HA 棄用、entry_source 移除）
>
> 本文件**與程式碼一比一對齊**：
> - 進場：`src/strategy/short_strategy.py`
...
| **WMA_fast / WMA_slow** | 原始 close 的加權移動平均，預設週期 = 4 / 6 | 進場交叉 |
| **swing high** | 原始 K 線「含當下、回看 N 根」最高價（N = `swing_lookback`，預設 4） | Stage 1 初始止損 |
| **Bollinger Band**（**原始** close） | 中軌 = WMA(close, 20)；上下軌 = 中軌 ± 2σ（σ 以中軌為基準） | Stage 3 拖曳止損 |

**注意**：
- ATR 不再用於止損（v2.0 Chandelier 已淘汰）。
- 止損相關計算（swing、Bollinger）一律用**原始 K 線**。

### 進場 K 線來源

> **重要**：v2.3（cffa883）起 Heikin-Ashi 完整棄用、`entry_source` 設定欄位移除。
> 所有 WMA / close 訊號**一律使用原始 K 線**。

---

## 二、建倉條件（Entry）

在 **Bar[t]**（=「訊號 bar」/ Bar[0]）收盤後判斷以下兩條件，皆成立則於 **Bar[t+1]**（Bar[1]）開盤掛限價空單。
...
即交叉前第 2、第 3 根之 close 皆高於交叉當根，確認交叉前已是下跌結構。
...
避免極端窄幅 K 線下 swing high 被往上推到比當下價還低，導致一進場立刻被掃。

### 進場動作（Bar[t+1] 開盤）

- 限價單賣出：`limit_price = Bar[t+1].open × (1 − slippage_pct)`（預設 0.03%；偏向下方確保 marketable）
- 撮合條件：`limit_price ≤ Bar[t+1].high` → 視為成交，`fill_price = max(limit_price, Bar[t+1].low)`
...
... | Bar[t-3] | Bar[t-2] | Bar[t-1] | Bar[t] (=Bar[0]) 死叉發生 | Bar[t+1] (=Bar[1]) 進場 |
       ↑           ↑                          ↑                          ↑
   Close[t-3]  Close[t-2]              條件1+2 收盤判斷           limit = open × 0.9997
   需 > Close[t]  需 > Close[t]
```
...
止損採**狀態機式三階段**，所有階段共通：**ratchet（空單只能下移）**。
...
```
swing_high   = max(high) over [t - swing_lookback + 1 .. t]   # 含訊號 bar
initial_stop = swing_high × (1 + stage1_slippage_buffer)
R            = initial_stop − entry_price                     # > 0
```

- 用 swing high 提供寬鬆初始保護；0.03% buffer 推離極值
...
```
progress_r = (entry_price − bar.low) / R
```
...
```
candidate = min(stage2_stop,  bb_upper[bar_index])
            其中 bb_upper = WMA(原始 close, 20) + 2σ
```

- Stage 2 fixed 永遠當作 ceiling：BB 暫時高於保本價時，stop 不上退
- BB 隨價格下跌 → candidate 自然下移 → ratchet 接管
- 若 `bb_upper` 為 NaN（暖機期）→ 退回 stage2_stop
...
```mermaid
flowchart TD
    A[Bar t 收盤] --> B{條件 1<br/>WMA_fast 下穿 WMA_slow？}
    B -- 否 --> A
    B -- 是 --> C{條件 2<br/>Close t-2 > Close t<br/>且 Close t-3 > Close t？}
    C -- 否 --> A
    C -- 是 --> D{條件 3<br/>initial_stop > raw_close t？}
    D -- 否 --> A
    D -- 是 --> E[Bar t+1 開盤<br/>limit = open × 0.9997<br/>過槓桿/越界檢查]
    E -- 拒絕 --> A
    E -- 成交 --> F[Stage 1 啟動<br/>R = swing_high×1.0003 − entry]
    F --> G{R 模式判斷}
    ...
    I -- 否 --> M{盤中觸 stop？}
    I -- 是 --> J[Stage 2<br/>stop = entry × 0.9990 − 0.2R]
    J --> K{progress_r ≥ stage3_trigger?}
    K -- 否 --> M
    K -- 是 --> L[Stage 3<br/>candidate = min stage2_stop, BB_upper<br/>ratchet 下移]
    L --> M
    M -- 否 --> H
    M -- 是 --> N[市價平倉]
    N --> A
...
| 項目 | v2.0（已棄用） | v2.1 | v2.2 | v2.3（本版） |
|------|---------------|------|------|--------------|
| 止損型態 | 單一 Chandelier | 三階段狀態機 | 同 v2.1 | 同 v2.1（外加 r_ladder 模式為預設） |
| Stage 2 公式 | — | `entry × (1 − 2×taker − slippage) − 0.2R` | `entry × (1 − 2×taker) − 0.2R`（滑點不重複計） | 同 v2.2 |
| `entry_source` | 隱含 HA | 隱含 HA | `ha` / `raw` 切換 | **完整移除**；一律 raw |
| Config 文件 | atr_*** | trailing.*** | 補 `account` / `fees` | 新增 `chop_filter`、`risk_per_trade_pct`、`source_dir` |
